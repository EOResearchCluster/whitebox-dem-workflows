...
whitebox_tools -r=EdgeDensity \
  --dem="$DEM" \
  -o="$OUTPUT_DIR/edge_density.tif"

# Embankment mapping
echo "  - Mapping embankments..."
whitebox_tools -r=EmbankmentMapping \
  --dem="$DEM" \
...
whitebox_tools -r=FeaturePreservingSmoothing \
  --dem="$DEM" \
...
whitebox_tools -r=GaussianFilter \
  --input="$DEM" \
...
whitebox_tools -r=DownslopeIndex \
  --dem="$DEM" \
...
whitebox_tools -r=MaxUpslopeElevChange \
  --dem="$DEM" \
...
whitebox_tools -r=NumUpslopeNeighbours \
  --dem="$DEM" \
