...
whitebox_tools -r=BreachDepressions \
  --dem="$DEM" \
...
whitebox_tools -r=DInfPointer \
  --dem="$OUTPUT_DIR/dem_breached.tif" \
  -o="$OUTPUT_DIR/dinf_pointer.tif"

# FD8 flow pointer
echo "  - FD8 flow pointer..."
whitebox_tools -r=Fd8Pointer \
  --dem="$OUTPUT_DIR/dem_breached.tif" \
...
whitebox_tools -r=Fd8FlowAccumulation \
  --dem="$OUTPUT_DIR/dem_breached.tif" \
...
whitebox_tools -r=MaxUpslopeFlowpathLength \
  --d8_pntr="$OUTPUT_DIR/d8_pointer.tif" \
...
whitebox_tools -r=StreamPowerIndex \
  --sca="$OUTPUT_DIR/d8_flow_accum_sca.tif" \
  --slope="outputs/02_geomorphometry/slope_degrees.tif" \
...
whitebox_tools -r=SedimentTransportIndex \
  --sca="$OUTPUT_DIR/d8_flow_accum_sca.tif" \
  --slope="outputs/02_geomorphometry/slope_degrees.tif" \
...
whitebox_tools -r=FindNoFlowCells \
  --dem="$OUTPUT_DIR/dem_breached.tif" \
  -o="$OUTPUT_DIR/no_flow_cells.tif"

# Edge contamination
echo "  - Detecting edge contamination..."
whitebox_tools -r=EdgeContamination \
  --dem="$OUTPUT_DIR/dem_breached.tif" \
  --flow_accum="$OUTPUT_DIR/d8_flow_accum.tif" \
